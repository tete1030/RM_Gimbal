...
float _incremental_control(PID_Controller *pidc, float cur_value, float target_value, float other, float *incre_output, uint8_t ignore_static_offset)
{
    float output;
    ...
    if(!ignore_static_offset)
    	pidc->last_output = output;

    return output;
};

float _absolute_control(PID_Controller *pidc, float cur_value, float target_value, float other, uint8_t ignore_static_offset)
{
    float output;
    pidc->pre_error[pidc->cur_error_offset] = target_value - cur_value;
...
float PID_Controller_Calc(PID_Controller *pidc, float cur_value, float target_value, float other, float *incre_output, uint8_t ignore_static_offset)
{
    if(pidc->config.mode == PID_Controller_Mode_Incremental)
    {
        return _incremental_control(pidc, cur_value, target_value, other, incre_output, ignore_static_offset);
    }
    else
    {
        return _absolute_control(pidc, cur_value, target_value, other, ignore_static_offset);
    }
}
